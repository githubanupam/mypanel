          ...
          <div class="tile">
            ...
            <div class="tile-body">
                ...
                <div class="input_fields_wrap">                  
                  <div class="row">
                    <h5 class="col-8 text-primary">Add Action</h5>
                    <div class="col-4 text-right text-primary add_field_button"><i class="fa fa-2x fa-plus-square"></i></div>
                  </div>
                  
                    <?php 
                        if (isset($page_action)) {

                          foreach ($page_action as $row_action) 
                          {
                    ?>
                            <div class="row">
                              <div class="col-md-5">
                                <select name="action[]" class="form-control">
                                  <option value="1" <?=($row_action['actionId']=='1')?"selected":""?>>View</option>
                                  <option value="2" <?=($row_action['actionId']=='2')?"selected":""?>>Edit</option>
                                  <option value="3" <?=($row_action['actionId']=='3')?"selected":""?>>Delete</option>
                                </select>
                              </div>
                              <div class="col-md-5">                    
                                <input type="text" name="action_name[]" value="<?=$row_action['actionName']?>" class="form-control" placeholder="action name"/>
                              </div>
                              <div class="text-right col-md-2" style="margin: auto;">
                                <i class="fa fa-2x fa-minus-circle remove_field text-danger"></i>
                              </div>
                            </div>
                    <?php
                          }
